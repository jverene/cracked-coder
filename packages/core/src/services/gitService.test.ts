...
import { CRACKED_DIR, homedir as pathsHomedir } from '../utils/paths.js';
...
describe('GitService', () => {
  ...
  describe('setupShadowGitRepository', () => {
    ...
    beforeEach(async () => {
      repoDir = path.join(homedir, CRACKED_DIR, 'history', PROJECT_SLUG);
    ...
    it('should create a .gitconfig file with the correct content', async () => {
      ...
      const expectedConfigContent =
        '[user]\n  name = Cracked Coder\n  email = cracked-coder@google.com\n[commit]\n  gpgsign = false\n';
