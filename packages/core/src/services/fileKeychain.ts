...
export class FileKeychain implements Keychain {
  ...
  constructor() {
    const configDir = path.join(homedir(), CRACKED_DIR);
    this.tokenFilePath = path.join(configDir, 'gemini-credentials.json');
    this.encryptionKey = this.deriveEncryptionKey();
  }

  private deriveEncryptionKey(): Buffer {
    const salt = `${os.hostname()}-${os.userInfo().username}-cracked-coder`;
    return crypto.scryptSync('cracked-coder-oauth', salt, 32);
  }
