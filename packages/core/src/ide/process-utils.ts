 ...
 * process).
 *
 * This function can be overridden by setting the `CRACKED_CODER_IDE_PID`
 * environment variable. This is useful for launching Cracked Coder in a
 * standalone terminal while still connecting to an IDE instance.
 *
 * If `CRACKED_CODER_IDE_PID` is set, the function uses that PID and fetches
 * the command for it.
 *
...
export async function getIdeProcessInfo(): Promise<{
  ...
  if (process.env['CRACKED_CODER_IDE_PID']) {
    const idePid = parseInt(process.env['CRACKED_CODER_IDE_PID'], 10);
