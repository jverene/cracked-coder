...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@cracked-coder/core/src/ide/detect-ide.js';

const CLI_IDE_COMPANION_IDENTIFIER = 'Google.cracked-coder-vscode-ide-companion';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Cracked Coder Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Cracked Coder IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'Cracked Coder Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('cracked-coder.runGeminiCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Cracked Coder.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run Cracked Coder in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `Cracked Coder (${selectedFolder.name})`,
    ...
    vscode.commands.registerCommand('cracked-coder.showNotices', async () => {
