...
function checkRelease() {
  try {
    ...
    const bucketUri = 'gs://cracked-coder-vscode-extension/release/1p/signed';
    ...
    const vsixFiles = files.filter((file) =>
      /signed-cracked-coder-vscode-ide-companion-\d+\.\d+\.\d+-[a-f0-9]{7}\.vsix$/.test(
        file,
      ),
    );
    ...
    const match =
      /signed-cracked-coder-vscode-ide-companion-(\d+\.\d+\.\d+)-([a-f0-9]{7})\.vsix$/.exec(
        fileName,
      );
