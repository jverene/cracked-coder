...
describe('ExtensionEnablementManager', () => {
  beforeEach(() => {
    ...
    vi.mocked(ExtensionStorage.getUserExtensionsDir).mockReturnValue(
      path.join(testDir.path, CRACKED_DIR),
    );
  ...
  it('should correctly prioritize more specific enable rules', () => {
    ...
    manager.enable('ext-test', true, '/Users/chrstn/cracked-coder');

    expect(manager.isEnabled('ext-test', '/Users/chrstn/cracked-coder')).toBe(
      true,
    );
  });
