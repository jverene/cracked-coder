...
import { writeToStderr } from '@cracked-coder/core';
...
describe('resolveWorkspacePolicyState', () => {
  ...
  beforeEach(() => {
    ...
    // Redirect CRACKED_CODER_HOME to the temp directory to isolate integrity storage
    vi.stubEnv('CRACKED_CODER_HOME', tempDir);

    workspaceDir = path.join(tempDir, 'workspace');
    fs.mkdirSync(workspaceDir);
    policiesDir = path.join(workspaceDir, '.cracked', 'policies');
  ...
  it('should not return workspace policies if cwd is the home directory', async () => {
    const policiesDir = path.join(tempDir, '.cracked', 'policies');
    fs.mkdirSync(policiesDir, { recursive: true });
    fs.writeFileSync(path.join(policiesDir, 'policy.toml'), 'rules = []');
  ...
  it('should return empty state if cwd is a symlink to the home directory', async () => {
    ...
    const symlinkDir = path.join(
      os.tmpdir(),
      `cracked-coder-symlink-${Date.now()}`,
    );
