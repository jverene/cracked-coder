...
describe('initializeOutputListenersAndFlush', () => {
  ...
  it('should flush backlogs and setup listeners if no listeners exist', async () => {
    const { coreEvents } = await import('@cracked-coder/core');
    const { initializeOutputListenersAndFlush } = await import('./cracked.js');
...
describe('getNodeMemoryArgs', () => {
  ...
  it('should return empty array if CRACKED_CODER_NO_RELAUNCH is set', () => {
...
describe('gemini.tsx main function kitty protocol', () => {
  ...
  beforeEach(() => {
    ...
    originalEnvNoRelaunch = process.env['CRACKED_CODER_NO_RELAUNCH'];
    process.env['CRACKED_CODER_NO_RELAUNCH'] = 'true';
  ...
  afterEach(() => {
    ...
    if (originalEnvNoRelaunch !== undefined) {
      process.env['CRACKED_CODER_NO_RELAUNCH'] = originalEnvNoRelaunch;
    } else {
      delete process.env['CRACKED_CODER_NO_RELAUNCH'];
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
describe('startInteractiveUI', () => {
  ...
  it('should enable mouse events when alternate buffer is enabled', async () => {
    const { enableMouseEvents } = await import('@cracked-coder/core');
    await startTestInteractiveUI(
      mockConfig,
      mockSettings,
      mockStartupWarnings,
  ...
  it('should perform all startup tasks in correct order', async () => {
    const { getVersion } = await import('@cracked-coder/core');
  ...
  it('should not recordSlowRender when less than threshold', async () => {
    const { recordSlowRender } = await import('@cracked-coder/core');
