...
import { getPackageJson, debugLogger } from '@cracked-coder/core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of Cracked Coder is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `Cracked Coder update available! ${currentVersion} → ${latestUpdate}`;
