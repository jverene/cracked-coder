...
to the Cracked Coder Policy Engine.
...
1.  Link this extension to your local Cracked Coder installation:

    ```bash
    gemini extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your Cracked Coder session.
...
For security, Cracked Coder ignores any `allow` decisions or `yolo` mode
