...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-cracked-coder/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-cracked-coder release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest run-cracked-coder release on GitHub.`,
    );
  }
};
