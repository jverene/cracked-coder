...
import { debugLogger, isGitRepository } from '@cracked-coder/core';
...
export function getInstallationInfo(
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        const brewPrefix = childProcess
          .execSync('brew --prefix cracked-coder', {
            encoding: 'utf8',
        ...
        // Brew is not installed or cracked-coder is not installed via brew.
    ...
    if (
      ...
      const updateCommand = 'pnpm add -g @google/cracked-coder@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @google/cracked-coder@latest';
      return {
    ...
    if (realPath.includes('/.bun/install/global')) {
      const updateCommand = 'bun add -g @google/cracked-coder@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @google/cracked-coder@latest';
    return {
