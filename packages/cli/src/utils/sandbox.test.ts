...
import { FatalSandboxError, type SandboxConfig } from '@cracked-coder/core';
  ...
  describe('start_sandbox', () => {
    ...
    it('should mount volumes correctly', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.arrayContaining([
          ...
          '--volume',
          expect.stringMatching(/[\\/]home[\\/]user[\\/]\.cracked/),
        ]),
        expect.any(Object),
      );
    });
    ...
    it('should handle networkAccess: false in Docker', async () => {
      ...
      expect(execSync).toHaveBeenCalledWith(
        expect.stringContaining('network create --internal cracked-coder-sandbox'),
        expect.any(Object),
      );
      expect(spawn).toHaveBeenCalledWith(
        'docker',
        expect.arrayContaining(['--network', 'cracked-coder-sandbox']),
        expect.any(Object),
      );
    });
  ...
  describe('gVisor (runsc)', () => {
    it('should use docker with --runtime=runsc on Linux', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        1,
        'docker',
        expect.arrayContaining(['images', '-q', 'cracked-coder-sandbox']),
      );
