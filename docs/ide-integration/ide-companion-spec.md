# Cracked Coder companion plugin: Interface specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable
Cracked Coder's IDE mode. For VS Code, these features (native diffing, context
awareness) are provided by the official extension
([marketplace](https://marketplace.visualstudio.com/items?itemName=Google.cracked-coder-vscode-ide-companion)).
...
Cracked Coder and the IDE plugin communicate through a local communication channel.
...
For Cracked Coder to connect, it needs to discover which IDE instance it's running
...
- **Tie-breaking with environment variables (recommended):** For the most
  ...
  set the `CRACKED_CODER_IDE_SERVER_PORT` environment variable in the integrated
  ...
  `CRACKED_CODER_IDE_SERVER_PORT` variable to identify and connect to the correct
  window's server.
