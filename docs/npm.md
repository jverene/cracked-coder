...
This monorepo contains two main packages: `@google/cracked-coder` and
`@google/cracked-coder-core`.

## `@google/cracked-coder`

This is the main package for the Cracked Coder. It is responsible for the user
...
`@google/cracked-coder-core`. This means that whether a user installs the package
with `npm install -g @google/cracked-coder` or runs it directly with
`npx @google/cracked-coder`, they are using this single, self-contained executable.
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @google/cracked-coder`.
