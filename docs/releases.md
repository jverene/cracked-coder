...
package names beginning with `@cracked-coder/**` instead of `@google/**`.
...
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | @google/cracked-coder            | @cracked-coder/cracked-coder                 |
| Core       | @google/cracked-coder-core       | @cracked-coder/cracked-coder-core A2A Server |
| A2A Server | @google/cracked-coder-a2a-server | @cracked-coder/cracked-coder-a2a-server      |
...
The workflow will then run `npm dist-tag add` for the appropriate `cracked-coder`,
`cracked-coder-core` and `cracked-coder-a2a-server` packages, pointing the specified
...
- `npx -y @google/cracked-coder@latest --version` to validate the push worked as
  expected if you were not doing a rc or dev tag
- `npx -y @google/cracked-coder@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall @google/cracked-coder && npm uninstall -g @google/cracked-coder && npm cache clean --force &&  npm install @google/cracked-coder@<version>`
...
1.  Go to the
    [Actions tab](https://github.com/cracked-coder/cracked-coder/actions/workflows/release-manual.yml)
    of the repository.
...
`packages/cli/google-cracked-coder-0.1.6.tgz`).
...
- **What happens:** The `npm publish` command is run for the
  `@google/cracked-coder-core` and `@google/cracked-coder` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g @google/cracked-coder` will download these packages, and
  `npm` will handle installing the `@google/cracked-coder-core` dependency
...
1.  **The JavaScript bundle is created:**
    - **What happens:** The built JavaScript from both `packages/core/dist` and
      ...
      `cracked.js`). The `node-pty` library is excluded from this bundle as it
...
2.  **The `bundle` directory is assembled:**
    - **What happens:** A temporary `bundle` folder is created at the project
      root. The single `cracked.js` executable is placed inside it, along with
      other essential files.
    - **File movement:**
      - `cracked.js` (from esbuild) -> `bundle/cracked.js`
...
3.  **The GitHub release is created:**
    - **What happens:** The contents of the `bundle` directory, including the
      `cracked.js` executable, are attached as assets to a new GitHub Release.
    - **Why:** This makes the single-file version of the CLI available for
      direct download and enables the
      `npx https://github.com/cracked-coder/cracked-coder` command, which downloads
...
- **NPM:** Publishes standard, un-bundled Node.js packages. The primary artifact
  ...
  `@google/cracked-coder-core`.
- **GitHub release:** Publishes a single, bundled `cracked.js` file that contains
