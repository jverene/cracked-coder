...
const CACHE_FILE = '.cracked/hook-cache.json';
...
```bash
#!/usr/bin/env bash
LOG_FILE=".cracked/hooks/debug.log"
...
# Test the hook
cat test-input.json | .cracked/hooks/my-hook.sh
...
# Test the hook
Get-Content test-input.json | .\.cracked\hooks\my-hook.ps1
...
Cracked Coder uses exit codes for high-level flow control:
...
input=$(cat)
echo "$input" >> .cracked/hook-inputs.log
        ...
        "hooks": [
          {
            "name": "secret-scanner",
            "type": "command",
            "command": "$GEMINI_PROJECT_DIR/.cracked/hooks/block-secrets.sh",
...
```bash
chmod +x .cracked/hooks/*.sh
chmod +x .cracked/hooks/*.js
...
```bash
git add .cracked/hooks/
git add .cracked/settings.json
...
.cracked/hook-cache.json
.cracked/hook-debug.log
.cracked/memory/session-*.jsonl
...
| :---------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| **System**                    | Configured by system administrators (e.g., `/etc/cracked-coder/settings.json`, `/Library/...`). Assumed to be the **safest**. |
| **User** (`~/.cracked/...`)    | Configured by you. You are responsible for ensuring they are safe.                                                         |
| **Extensions**                | You explicitly approve and install these. Security depends on the extension source (integrity).                            |
| **Project** (`./.cracked/...`) | **Untrusted by default.** Safest in trusted internal repos; higher risk in third-party/public repos.                       |

#### Project Hook Security

When you open a project with hooks defined in `.cracked/settings.json`:

1. **Detection**: Cracked Coder detects the hooks.
...
> changed (e.g., by a `git pull`), its identity changes. Cracked Coder will treat
...
Hooks inherit the environment of the Cracked Coder process, which may include
sensitive API keys. Cracked Coder provides a
...
```bash
ls -la .cracked/hooks/my-hook.sh
chmod +x .cracked/hooks/my-hook.sh
```
...
echo "$GEMINI_PROJECT_DIR/.cracked/hooks/my-hook.sh"

# Verify file exists
test -f "$GEMINI_PROJECT_DIR/.cracked/hooks/my-hook.sh" && echo "File exists"
```
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. Cracked Coder
