...
Agent Skills extend Cracked Coder with specialized expertise. In this guide, you'll
...
1.  Run the following command to create the folders:
    ...
    **macOS/Linux**
    ...
    ```bash
    mkdir -p .cracked/skills/api-auditor/scripts
    ```

    **Windows (PowerShell)**

    ```powershell
    New-Item -ItemType Directory -Force -Path ".cracked\skills\api-auditor\scripts"
    ```

### Create the definition

1.  Create a file at `.cracked/skills/api-auditor/SKILL.md`. This tells the agent
...
1.  Create a file at `.cracked/skills/api-auditor/scripts/audit.js`. This is the
    code the agent will run.

    ```javascript
    // .cracked/skills/api-auditor/scripts/audit.js
...
Cracked Coder automatically discovers skills in the `.cracked/skills` directory. You
...
`node .cracked/skills/api-auditor/scripts/audit.js http://geminili.com`
