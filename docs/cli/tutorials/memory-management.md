...
Control what Cracked Coder knows about you and your projects. In this guide, you'll
learn how to define project-wide rules with `CRACKED.md`, teach the agent
...
Out of the box, Cracked Coder is smart but generic. It doesn't know your preferred
...
## How to define project-wide rules (CRACKED.md)

The most powerful way to control the agent's behavior is through `CRACKED.md`
...
1.  In the root of your project, create a file named `CRACKED.md`.
...
3.  Start a new session. Cracked Coder will now know these rules automatically.
...
1.  **Global:** `~/.cracked/CRACKED.md` (Rules for _every_ project you work on).
2.  **Project Root:** `./CRACKED.md` (Rules for the current repository).
3.  **Subdirectory:** `./src/CRACKED.md` (Rules specific to the `src` folder).
...
`CRACKED.md` files and saved memories), use the `/memory show` command.
...
If you edit a `CRACKED.md` file while a session is running, the agent won't know
...
- **Keep it focused:** Don't dump your entire internal wiki into `CRACKED.md`.
...
- **Review often:** Periodically check your `CRACKED.md` files to remove outdated
  rules.
