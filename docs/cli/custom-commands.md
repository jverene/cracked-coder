...
prompts as personal shortcuts within Cracked Coder. You can create commands that
...
Cracked Coder discovers commands from two locations, loaded in a specific order:

1.  **User commands (global):** Located in `~/.cracked/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.cracked/commands/`. These commands are specific to the
...
- A file at `~/.cracked/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.cracked/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
```toml
# In: <project>/.cracked/commands/changelog.toml
...
When a custom command attempts to execute a shell command, Cracked Coder will now
...
````toml
# In: <project>/.cracked/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.crackedignore` if enabled.
...
```toml
# In: <project>/.cracked/commands/review.toml
...
```bash
mkdir -p ~/.cracked/commands/refactor
touch ~/.cracked/commands/refactor/pure.toml
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.cracked\commands\refactor"
New-Item -ItemType File -Force -Path "$env:USERPROFILE\.cracked\commands\refactor\pure.toml"
```

**2. Add the content to the file:**

Open `~/.cracked/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.cracked/commands/refactor/pure.toml
...
Cracked Coder will then execute the multi-line prompt defined in your TOML file.
