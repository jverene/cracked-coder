...
Agent Skills allow you to extend Cracked Coder with specialized expertise,
...
Unlike general context files ([`CRACKED.md`](./gemini-md.md)), which provide
...
Cracked Coder discovers skills from three primary locations:

1.  **Workspace Skills**: Located in `.cracked/skills/` or the `.agents/skills/`
    alias. Workspace skills are typically committed to version control and
    shared with the team.
2.  **User Skills**: Located in `~/.cracked/skills/` or the `~/.agents/skills/`
...
precedence over the `.cracked/skills/` directory. This generic alias provides an
...
# Discovers skills (SKILL.md or */SKILL.md) and creates symlinks in ~/.cracked/skills
# (or ~/.agents/skills)
gemini skills link /path/to/my-skills-repo

# Link to the workspace scope (.cracked/skills or .agents/skills)
...
# Uses the user scope by default (~/.cracked/skills or ~/.agents/skills)
...
# Install to the workspace scope (.cracked/skills or .agents/skills)
...
1.  **Discovery**: At the start of a session, Cracked Coder scans the discovery
