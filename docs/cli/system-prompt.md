...
The core system instructions that guide Cracked Coder can be completely replaced
...
via a `.cracked/.env` file. See
[Persisting Environment Variables](../get-started/authentication.md#persisting-environment-variables).

- Use the project default path (`.cracked/system.md`):
  - `GEMINI_SYSTEM_MD=true` or `GEMINI_SYSTEM_MD=1`
  - The CLI reads `./.cracked/system.md` (relative to your current project
    directory).
...
- Persist for a project using `.cracked/.env`:
  - Create `.cracked/system.md`, then add to `.cracked/.env`:
    - `GEMINI_SYSTEM_MD=1`
...
## Best practices: SYSTEM.md vs CRACKED.md
...
CRACKED.md focused on high‑level guidance and project specifics.

## Troubleshooting

- Error: `missing system prompt file '…'`
  - Ensure the referenced path exists and is readable.
  - For `GEMINI_SYSTEM_MD=1|true`, create `./.cracked/system.md` in your project.
- Override not taking effect
  - Confirm the variable is loaded (use `.cracked/.env` or export in your shell).
